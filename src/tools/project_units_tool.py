...
def get_project_units(
    ...
    if unit_code:
        filtered_units = [
            u for u in filtered_units if u.get("code", "").lower() == unit_code.lower()
        ]
    ...
    if unit_type:
        filtered_units = [
            ...
            if unit_type.lower() in u.get("unit_type", "").lower()
        ]
    ...
    if building:
        filtered_units = [
            u
            for u in filtered_units
            if building.lower() in u.get("building", "").lower()
        ]

    # Exact match for floor
    if floor:
        filtered_units = [
            u for u in filtered_units if u.get("floor", "").lower() == floor.lower()
        ]
    ...
    if availability:
        filtered_units = [
            u
            for u in filtered_units
            if u.get("availability", "").lower() == availability.lower()
        ]

    # Unit area range filters
    if min_area is not None:
        filtered_units = [
            u for u in filtered_units if _safe_float(u.get("unit_area", 0)) >= min_area
        ]
    if max_area is not None:
        filtered_units = [
            u for u in filtered_units if _safe_float(u.get("unit_area", 0)) <= max_area
        ]
    ...
    if price is not None:
        ...
        logging.info(
            f"Price filter: target={target_price}, range=[{min_price_range:.2f}, {max_price_range:.2f}]"
        )

    if min_price is not None:
        filtered_units = [
            u for u in filtered_units if _safe_float(u.get("price", 0)) >= min_price
        ]

    if max_price is not None:
        filtered_units = [
            u for u in filtered_units if _safe_float(u.get("price", 0)) <= max_price
        ]
    ...
    if sellable_area is not None:
        ...
        filtered_units = [
            ...
            for u in filtered_units
            if min_area_range
            <= _safe_float(u.get("sellable_area", 0))
            <= max_area_range
        ]
        logging.info(
            f"Sellable area filter: target={target_area}, range=[{min_area_range:.2f}, {max_area_range:.2f}]"
        )
    ...
    if min_sellable_area is not None:
        filtered_units = [
            u
            for u in filtered_units
            if _safe_float(u.get("sellable_area", 0)) >= min_sellable_area
        ]
    ...
    if max_sellable_area is not None:
        filtered_units = [
            u
            for u in filtered_units
            if _safe_float(u.get("sellable_area", 0)) <= max_sellable_area
        ]
    ...
    if unit_type_filter:
        filtered_units = [
            u
            for u in filtered_units
            if u.get("type", "").lower() == unit_type_filter.lower()
        ]

    logging.info(
        f"Tool: Returning {len(filtered_units)} units after filtering cached data."
    )
    ...
    if pick_random:
        if filtered_units:
            ...
            return [
                {"error": "No units matched the criteria to pick a random one from."}
            ]
    ...
    if len(filtered_units) > 10:
        ...
        summary.append(
            {"summary_message": f"Found {len(filtered_units)} units. Showing first 10."}
        )
        return summary
